# Curated AI Resources
A high-signal list of transformative projects, papers, and tools.

| Resource | Type | Description | URL |
|---|---|---|---|
| --- | --- | --- | --- |
| AI Waves Agents | Framework | Open-source framework for data-centric, self-evolving autonomous agents. | https://github.com/aiwaves-cn/agents |
| APEX-MEM | Memory System | Agentic semi-structured memory with temporal reasoning for long-term conversational AI. | https://arxiv.org/abs/2604.14362 |
| Agent Armor | Tool | Rust runtime for enforcing policies on AI agent actions. | https://github.com/EdoardoBambini/Agent-Armor-Iaga |
| Agent-Native OS | Concept | Cognitive resource orchestration (VCM, Scheduling, Semantic I/O). | See docs/resources/agent-native-os.md |
| Agentic Microphysics | Research | Framework for population-level risk and interaction-level safety in agentic AI. | https://arxiv.org/abs/2604.15236v1 |
| Autogenesis Protocol | Concept | Self-evolving agent protocol decoupling resources from evolution operators. | https://arxiv.org/abs/2604.15034v1 |
| Bindu | Tool | Framework for transforming AI agents into interoperable living microservices. | https://github.com/GetBindu/Bindu |
| Blue Data Intelligence Layer | System | Multi-source, multi-modal data-centric intelligence layer for agentic processing. | https://arxiv.org/abs/2604.15233v1 |
| CBCL | Research | Safe, self-extending agent communication framework for dynamic and verifiable protocol evolution. | https://arxiv.org/abs/2604.14512 |
| CascadeDebate | Research | Multi-agent deliberation at escalation boundaries to optimize test-time compute in LLM cascades. | https://arxiv.org/abs/2604.12262 |
| Claude Code Showcase | Case Study | Real-world examples of agentic workflows for triage, docs, and quality audits. | https://github.com/ChrisWiles/claude-code-showcase |
| CoTEvol | Research | Genetic evolutionary framework for self-evolving Chain-of-Thoughts in mathematical reasoning. | https://arxiv.org/abs/2604.14768 |
| Codel | Tool | Fully autonomous AI Agent with terminal, browser, and editor integration. | https://github.com/semanser/codel |
| CoopEval: Benchmarking Cooperation-Sustaining Mechanisms and LLM Agents in Social Dilemmas | It is increasingly important that LLM agents interact effectively and safely with other goal-pursuin... | Reveals a critical failure mode where higher reasoning leads to lower cooperation. Essential for des... | [arXiv:2604.15267](https://arxiv.org/abs/2604.15267) |
| Dr. RTL Skill Learning | Research | Group-relative skill distillation for autonomous tool improvement. | https://arxiv.org/abs/2604.14989v1 |
| Egregore | Tool | Shared memory and coordination for multiplayer Claude Code agents. | https://github.com/egregore-labs/egregore |
| El Agente Forjador | Research | Task-driven agent synthesis that automatically generates specialized agents with tailored toolsets. | https://arxiv.org/abs/2604.14609 |
| HemmaBo | Tool | MCP server implementation for agent-based vacation rentals. | https://github.com/HemmaBo-se/hemmabo-mcp-server |
| History LLMs | Tool | Time-locked models trained on pre-1913 texts to avoid hindsight contamination. | https://github.com/DGoettlich/history-llms |
| IE as Cache | Research | Repurposes Information Extraction as a cognitive cache to enhance agentic reasoning. | https://arxiv.org/abs/2604.14930 |
| IG-Search | 2604.15148 | Implements step-level Information Gain rewards for search-augmented reasoning, drastically reducing redundant queries. | [https://arxiv.org/abs/2604.15148](https://arxiv.org/abs/2604.15148) |
| LLM Judge Reliability | Research | Diagnostic toolkit using conformal prediction to quantify judge uncertainty. | https://arxiv.org/abs/2604.15302v1 |
| LLMs Gaming Verifiers: RLVR can Lead to Reward Hacking | As reinforcement Learning with Verifiable Rewards (RLVR) has become the dominant paradigm for scalin... | Warns against reward hacking in RLVR-trained models. This is a crucial insight for anyone implementi... | [arXiv:2604.15149](https://arxiv.org/abs/2604.15149) |
| Layered Mutability | Research | Framework for governance in persistent self-modifying agents, warning against compositional drift. | https://arxiv.org/abs/2604.14717 |
| NornicDB | Tool | Distributed low-latency Graph+Vector database with MCP server integration. | https://github.com/orneryd/NornicDB |
| OpenCode | Tool | Open source alternative to proprietary coding agents with a server/client architecture. | https://opencode.ai/ |
| SRMU (Relevance-Gated Memory) | 2604.15121 | A domain-agnostic update rule for streaming memories that filters redundant/stale info using relevance gating. | [https://arxiv.org/abs/2604.15121](https://arxiv.org/abs/2604.15121) |
| SWE-TRACE | Research | Optimizes long-horizon SWE agents through rubric-based process reward models and test-time scaling. | https://arxiv.org/abs/2604.14820 |
| Samyama Graph | Tool | High-efficiency graph-vector database capable of querying 1B edges for $2.50. | https://github.com/samyama-ai/samyama-graph |
| Scepsy | System | Serving framework for agentic workflows using aggregate LLM pipelines to optimize throughput and latency. | https://arxiv.org/abs/2604.15186 |
| Self-Evolved ABC | Case Study | Multi-agent autonomous evolution of a million-line EDA codebase. | https://arxiv.org/abs/2604.15082v1 |
| Stability and Generalization in Looped Transformers | Looped transformers promise test-time compute scaling by spending more iterations on harder problems... | Explores test-time compute scaling through looped architectures. This is a key frontier for LLM reas... | [arXiv:2604.15259](https://arxiv.org/abs/2604.15259) |
| TTC-Scaling | Research | Scaling laws for test-time compute (TTC); identifies the $\text{Performance} \propto \text{TTC}^\alpha$ power law and the Verifier Bottleneck. | See docs/artifacts/TTC-Scaling.md |
| WaooWaoo | Platform | Industry-first professional AI Agent platform for controllable film & video production. | https://github.com/saturndec/waoowaoo |
| llms.txt | Concept | A proposal for a standard way for websites to provide LLM-friendly descriptions and guides. | https://annas-archive.li/blog/llms-txt.html |
